...
def getCryptoPriceDay(coin, currency):
    price = get_historical_price(
        coin, currency, (datetime.datetime.now() - datetime.timedelta(days=1))
    )
    return price
...
def index(request):
    if request.user.is_authenticated == False:
        ...
        if all_wallet_item_names != []:
            prices = getCryptoPrice(all_wallet_item_names, CURRENCY_VALUE)
            dayPrices = {}
            for i in all_wallet_item_names:
                dayPrices[i] = getCryptoPriceDay(i, CURRENCY_VALUE)[
                    all_wallet_items.get(content=i).content
                ][CURRENCY_VALUE]

            for i in all_wallet_items:
                i.price = prices[i.content][CURRENCY_VALUE]
                i.value = round(prices[i.content][CURRENCY_VALUE] * i.amount, 2)
        ...
        if prices is not None:
            for i in prices:
                total += (
                    prices[i][CURRENCY_VALUE] * all_wallet_items.get(content=i).amount
                )
        total = round(total, 2)
...
def addWalletView(request):
    ...
    if get_price(x, CURRENCY_VALUE) is not None:
        if x != "" and WalletItem.objects.filter(user=request.user, content=x).exists():
            ...
            if amt != "":
                existing_amt = WalletItem.objects.get(
                    user=request.user, content=x
                ).amount
                WalletItem.objects.filter(user=request.user, content=x).update(
                    amount=existing_amt + amt
                )
                messages.success(
                    request, "Added " + str(amt) + " " + x + " to your wallet"
                )
            else:
